        ...
        this.comments = [], this.tokens = function*(input) {
          ...
          for (;input.length; ) {
            ...
            line++, col = 0, yield buildToken(TOKEN_TYPES_newline); else if ("#" === input[0]) {
              ...
              for (var indent = "", i = 0; " " === input[i]; i++) indent += input[i];
              ...
              for (var _val = "", _i = 0; ;_i++) {
                var currentChar = input[_i];
                if (_val += currentChar, _i > 0 && '"' === currentChar && ("\\" !== input[_i - 1] || "\\" === input[_i - 2])) break;
              }
              chop = _val.length;
              try {
              ...
              for (var _val2 = "", _i2 = 0; /^[0-9]$/.test(input[_i2]); _i2++) _val2 += input[_i2];
            ...
            chop++; else if (/^[a-zA-Z\/-]/g.test(input)) {
              for (var name = "", _i3 = 0; _i3 < input.length; _i3++) {
                var char = input[_i3];
