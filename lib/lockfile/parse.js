...
var _require = require('js-yaml'), safeLoad = _require.safeLoad, FAILSAFE_SCHEMA = _require.FAILSAFE_SCHEMA;
...
class Parser {
  constructor(input, fileLoc) {
    if (fileLoc === void 0) fileLoc = 'lockfile';
    this.token = void 0;
      ...
      if (version > _constants.LOCKFILE_VERSION) {
        throw new _errors.MessageError(
          `Can't install from a lockfile of version ${version} as you're on an old yarn version that only supports ` +
            `versions up to ${_constants.LOCKFILE_VERSION}. Run \`$ yarn self-update\` to upgrade to the latest version.`
        );
      }
    }
  ...
  unexpected(msg) {
    if (msg === void 0) msg = 'Unexpected token';
