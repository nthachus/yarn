    ...
    while (true) {
      ...
      if (propToken.type === TOKEN_TYPES.newline) {
        ...
        while (this.token.type === TOKEN_TYPES.comma) {
          ...
          (0, _invariant.default)(_key, 'Expected a key');
          keys.push(_key);
          this.next();
        }
        ...
        this.unexpected(`Unknown token: ${_util.default.inspect(propToken)}`);
      }
    }
...
function parse(str, fileLoc) {
  ...
  try {
    var result = (0, _jsYaml.safeLoad)(str, {schema: _jsYaml.FAILSAFE_SCHEMA});
